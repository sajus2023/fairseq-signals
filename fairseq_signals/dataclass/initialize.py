...
def hydra_init(cfg_name = "config") -> None:
    ...
    for k in Config.__dataclass_fields__:
        field_info = Config.__dataclass_fields__[k]
        # Use default_factory if default is MISSING
...
def add_defaults(cfg: DictConfig) -> None:
    ...
    from fairseq_signals.utils.registry import REGISTRIES
    from fairseq_signals.tasks import TASK_DATACLASS_REGISTRY
    from fairseq_signals.models import ARCH_MODEL_NAME_REGISTRY, MODEL_DATACLASS_REGISTRY
    from fairseq_signals.dataclass.utils import merge_with_parent
